...
@section('page-heading')
    ...
    <h3 class="page-title"> Edit Fasilitas</h3>
@endsection
            ...
            <div class="portlet light bordered">
                <div class="portlet-title">
                    <div class="caption font-dark">
                        <span class="caption-subject bold uppercase"> Edit Fasilitas</span>
                    </div>
                        ...
                        <div class="tab-content" style="padding: 30px;">
                            @foreach(config('app.all_langs') as $lang)
                                <div class="tab-pane fade @if($lang == config('app.default_locale')) active in @endif" id="{{ 'tab_'.$lang }}">
                                    {!! Form::model($fasilitas->where('lang', $lang)->first(), ['route' => ['config.fasilitas.update', $fasilitas->where('lang', $lang)->first()->equal_id],'method' =>'patch','files' => true])!!}
                                    ...
                                    @if($lang == config('app.default_locale'))
                                        <div class="form-group {!! $errors->{$lang}->has('thumb_image') ? 'has-error' : '' !!}">
                                            ...
                                            @if (!empty($fasilitas->where('lang', $lang)->first()) && $fasilitas->where('lang', $lang)->first()->thumb_image !== '')
                                                <div class="row" style="margin-top: 15px;">
                                                    <div class="col-md-4">
                                                        <p style="margin: 5px 0;">Current Thumbnail Image:</p>
                                                        <div class="thumbnail">
                                                            <img src="{{ $fasilitas->where('lang', $lang)->first()->article_thumb_image }}" class="img-rounded">
                                                        </div>
                                                    </div>
                                                </div>
                                            @endif
                                    ...
                                    <div class="text-right">
                                        {!! Form::submit(empty($fasilitas->where('lang', $lang)->first()) ? 'Save' : 'Update', ['class'=>'btn btn-primary']) !!}
                                    </div>
                                    {!! Form::close() !!}
                                </div>
                            @endforeach
