...
@section('page-heading')
    <div class="page-bar">
        <ul class="page-breadcrumb">
            ...
            <li>
                <a href="{{ route('config.team.index') }}"><span>Team</span></a>
                <i class="fa fa-circle"></i>
            </li>
            <li>
                <span>Edit Team</span>
        ...
        <div class="col-md-12">
            ...
            <div class="portlet light bordered">
                ...
                <div class="portlet-body">
                    {!! Form::open(['route' => ['config.team.update', $team->first()->equal_id], 'method' => 'patch', 'files' => true, 'id' => 'submit_form', 'novalidate'])!!}
                    @foreach(config('app.all_langs') as $lang)
                        <div class="form-group {!! $errors->has('name_'.$lang) ? 'has-error' : '' !!}">
                            ...
                            {!! Form::text('name_'.$lang, $team->where('lang', $lang)->first()->name, ['class'=>'form-control', 'required']) !!}
                            {!! $errors->first('name_'.$lang, '<p class="help-block">:message</p>') !!}
                        </div>
                        <div class="form-group {!! $errors->has('conten_'.$lang) ? 'has-error' : '' !!}">
                            {!! Form::label('conten_'.$lang, 'Team Conten ('.config('app.human_langs')[$lang].')') !!}
                            {!! Form::textarea('conten_'.$lang, $team->where('lang', $lang)->first()->conten, ['class'=>'form-control editor-textarea']) !!}
                    ...
                    <div class="form-group {!! $errors->has('image') ? 'has-error' : '' !!}">
                        ...
                        @if (!empty($team->first()) && $team->first()->image !== '')
                            <div class="row" style="margin-top: 15px;">
                                <div class="col-md-8">
                                    <p style="margin: 5px 0;">Current Image:</p>
                                    <div class="thumbnail">
                                        <img src="{{ $team->first()->home_slider }}" class="img-rounded">
                                    </div>
                                </div>
                            </div>
                        @endif
